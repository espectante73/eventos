  ...
  return (
    new Promise((resolve) => {
        ...
        const dibujarDatosGenerales = (ctx, W, H) => {
          ...
          // Ni con la fecha sin confirmar: una invitación con una fecha
          // que puede cambiar es peor que una sin fecha.
          const hayFecha = evento.fecha && evento.imprimirFecha !== false && !evento.fechaSinConfirmar;
          const fechaValor = hayFecha ? formatearFecha(evento.fecha) : "";
          const diaSemanaValor =
            hayFecha ? formatearDiaSemana(evento.fecha) : "";
